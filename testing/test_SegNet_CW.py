...
class test_model():
    ...
    def test_total_image(self, test_dataset, img_NO, model_img, model_addition, predict_path):
        ...
        with torch.no_grad():
            ...
            prio_features = model_addition(prio_input)
            
            prio_features_list = [prio_features[0], prio_features[1], prio_features[2], prio_features[3], prio_features[4]]
            
            out = model_img(img, prio_features_list).cpu().detach().numpy()[0]

        image_predect = np.argmax(out, 0).astype(np.uint8)
    ...
    def test_single_patch(self, test_dataset, start_h, end_h, start_w, end_w, model_img, model_addition, model_dis):
        with torch.no_grad():
            ...
            prio_input = torch.cat([dsm_patch, dis_patch, ndvi_patch], 1)
            prio_input = torch.cat([prio_input, prio_input, prio_input, prio_input, prio_input], 0)
            prio_features = model_addition(prio_input)
            batch_size = 1
            ...
            out = model_img(img_patch, prio_features_list)
            
            max_probability, max_index = torch.max(out, 1)
...
if __name__ == "__main__":
    ...
    for i in range(50, 0, -1):
        mlp_path= '..\\XZY_DeepLearning_Framework\\result\\add_model' +str(i)+'.pkl'
