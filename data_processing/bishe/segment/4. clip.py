...
class Clip():
    ...
    def clip_coupleIMG(self, img1_path, img2_path, out_folder, step, overlap=0, bk_value1:list=None, bk_value2:list=None, bk_rate=0.2, datatype=gdal.GDT_UInt16, start_imgnum=0): 
        ...
        for start_x in range(0, img1.shape[1], step-overlap):
            for start_y in range(0, img1.shape[2], step-overlap):
                ...
                gdal_write_tif(img2_outname, img2_split, step, step, datatype=1)
    ...
    def clip_HSIMG(self, img_path, lab_path, out_folder, half_window_size=5, bk_value=-1):
        '''
        ...
        if os.path.exists(out_folder)==False:
            ...
            return
        # 判断图像对大小是否一致
        img, [h1, w1, _, _, _, img_gdal] = gdal_read_tif(img_path)
        lab, [h2, w2, _, _, _, _] = gdal_read_tif(lab_path)
        if w1!=w2 or h1!=h2:
            print('Img size not match')
            return
        # /////////////////////////////////////////////////////////////
        ...
        # # /////////////////////////////////////////////////////////////
        # 读写最大值最小值
        max_list = get_metadata(img_gdal, 'max')
        min_list = get_metadata(img_gdal, 'min')
        if max_list and min_list:
            max_list = list(map(int, max_list.strip().split()))
            min_list = list(map(int, min_list.strip().split()))
            print('MaxMin: Using the original maxmin in Metadata of (%s)'%img_path)
        else:
            max_list = []
            min_list = []
            for b in range(img.shape[0]):
                ...
                max_list.append(max)
                min_list.append(min)
            print('MaxMin: Computing the maxmin of (%s)'%img_path)
        add_metadata = {'max':max_list, 'min':min_list}
        # /////////////////////////////////////////////////////////////
        # 开始裁剪
        line_num = 0
        for x in range(lab.shape[0]):
            for y in range(lab.shape[1]):
                if lab[x, y] != bk_value:
                    ...
                    if end_y >= lab.shape[1]:
                        end_y = lab.shape[1] - 1
                        start_y = end_y - 2*half_window_size - 1

                    if end_x >= lab.shape[0]:
                        end_x = lab.shape[0] - 1
                    ...
                    outname = outimg_folder + str(line_num) + '.tif'
                    gdal_write_tif(outname, img_clip, img_clip.shape[1], img_clip.shape[2], img_clip.shape[0], datatype=2, add_metadata=add_metadata)

                    # str1 = str(outname) + ',' + str(lab[x, y]-1) + '\n'           #没有第0类背景类，所有类往前进一个
                    str1 = str(outname) + ',' + str(lab[x, y]) + '\n'           #有第0类背景类，所有类保持不变
                    # print(str1, start_x, end_x, start_y, end_y)
                    train_csv.write(str1)

                    line_num+=1
    def clip_coupleIMG_withpt(self, img_path, lab_path, ptimg_path, out_folder, half_window_size=5, bk_value=-1):
        '''
            针对均匀抽样点的裁剪方式：输入样本点数据（lab_path），对存在样本点的像素以其为窗口中心进行裁剪
        '''
        # /////////////////////////////////////////////////////////////
        # 判断文件和文件夹是否存在
        if os.path.exists(img_path)==False:
        ...
        img, [h1, w1, _, _, _, img_gdal] = gdal_read_tif(img_path)
        lab, [h2, w2, _, _, _, _] = gdal_read_tif(lab_path)
        ptimg, [h3, w3, _, _, _, _] = gdal_read_tif(ptimg_path)
        img1, img2 = img[4,:,:],  img[4+6,:,:]
        img1, img2 = img1[np.newaxis,:,:], img2[np.newaxis,:,:]
        img = np.concatenate([img1, img2], 0)
        lab = lab[4,:,:]
        if w1!=w2 or h1!=h2 or w3!=w1 or h3!=h1:
            print('Img size not match')
            return
        # /////////////////////////////////////////////////////////////
        # 新建输出文件夹及输出文件
        ...
        # 开始裁剪
        line_num = 0
        for x in range(ptimg.shape[0]):
            for y in range(ptimg.shape[1]):
                if ptimg[x, y] != bk_value:
                    start_x = x - half_window_size
                    start_y = y - half_window_size
                    end_x = x + half_window_size + 1
                    end_y = y + half_window_size + 1
                    ...
                    img_clip = img[:, start_x:end_x, start_y:end_y]
                    lab_clip = lab[start_x:end_x, start_y:end_y]
                    outimg_name = outimg_folder + str(line_num) + '.tif'
                    outlab_name = outlab_folder + str(line_num) + '.tif'
                    gdal_write_tif(outimg_name, img_clip, img_clip.shape[1], img_clip.shape[2], img_clip.shape[0], datatype=2, add_metadata=add_metadata)
                    gdal_write_tif(outlab_name, lab_clip, lab_clip.shape[0], lab_clip.shape[1], 1                , datatype=2, add_metadata=add_metadata)
                    str1 = str(outimg_name) + ',' + str(outlab_name) + '\n'               
                    csv_file.write(str1)
                    line_num+=1
...
if __name__ == '__main__':
    ...
    img2_path = 'E:\\dataset\\毕设数据\\new\\2. MS\\SS_month_Pixs_500\\9_12\\lab_train.tif'
    img3_path = 'E:\\dataset\\毕设数据\\new\\2. MS\\SS_month\\labelmonth_layerstack.tif'
    out_folder = 'E:\\dataset\\毕设数据\\new\\2. MS\\SS_month_Pixs_500\\9_12\\'
    clip.clip_coupleIMG_withpt(img1_path, img3_path, img2_path, out_folder, half_window_size=128, bk_value=255)
