...
Hed_IMG_pre = HED.HED(input_channels=3, out_channels=6).cuda()
Hed_IMG = HED.HED(input_channels=4, out_channels=6).cuda()
...
for epoch in range(201):
    ...
    for i, sample in enumerate(train_dataloader, 0):
        ...
        ##################################################################
        loss_side1 = HED_Loss.HED_LOSS_WITH_DISTANCE_AND_CLASS_RATE_MUTIL_TASK_GENERAL(img_sideout1, labels, labels_class, img_sideout1.shape[1])
        if loss_side1 == False:
            continue
        loss_side2 = HED_Loss.HED_LOSS_WITH_DISTANCE_AND_CLASS_RATE_MUTIL_TASK_GENERAL(img_sideout2, labels, labels_class, img_sideout2.shape[1])
        loss_side3 = HED_Loss.HED_LOSS_WITH_DISTANCE_AND_CLASS_RATE_MUTIL_TASK_GENERAL(img_sideout3, labels, labels_class, img_sideout3.shape[1])
        loss_side4 = HED_Loss.HED_LOSS_WITH_DISTANCE_AND_CLASS_RATE_MUTIL_TASK_GENERAL(img_sideout4, labels, labels_class, img_sideout4.shape[1])
        loss_side5 = HED_Loss.HED_LOSS_WITH_DISTANCE_AND_CLASS_RATE_MUTIL_TASK_GENERAL(img_sideout5, labels, labels_class, img_sideout5.shape[1])
        loss_fuse = HED_Loss.HED_LOSS_WITH_DISTANCE_AND_CLASS_RATE_MUTIL_TASK_GENERAL(img_sideoutfuse, labels, labels_class, img_sideout5.shape[1])
