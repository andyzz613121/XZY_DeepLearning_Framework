...
from model.SegNet.SegNet_CW import SegNet_ADD, SegNet_IMG, add_pre_model
from model.SegNet.SegNet import SegNet as SegNet_Basic
def main():
    ...
    model_img_pre = SegNet_Basic(3, 6).cuda()
    model_img_pre.load_state_dict(torch.load('pretrained\\SegNet_CW_Pretrained\\SegNet_CW_Pretrained_Vai.pth'))
    model_img = SegNet_IMG(3, 6).cuda()
    model_img = add_pre_model(model_img, model_img_pre)
    model_add = SegNet_ADD(3, 6).cuda()
    ...
    optimizer = torch.optim.SGD(params=[
        ...
        {'params': model_add.parameters(), 'lr': 1*init_lr},
    ...
    while True: #cur_itrs < opts.total_itrs:
        ...
        for i, sample in enumerate(train_loader, 0):
            ...
            prio_input = torch.cat([dsm, dis, ndvi], 1)
            prio_input = torch.cat([prio_input, prio_input, prio_input, prio_input, prio_input], 0)
            prio_features = model_add(prio_input)
            prio_features_list = [prio_features[0:batch_size], prio_features[batch_size:2*batch_size], prio_features[2*batch_size:3*batch_size], 
            prio_features[3*batch_size:4*batch_size], prio_features[4*batch_size:5*batch_size]]
            
            out = model_img(img, prio_features_list)

            loss = Classification_Loss.loss_with_class(out, label)
