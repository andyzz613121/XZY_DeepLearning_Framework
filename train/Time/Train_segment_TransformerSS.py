...
def main(netmodel):
    ...
    while cur_epochs <= total_epoch: #cur_itrs < opts.total_itrs:
        ...
        for i, sample in enumerate(train_loader, 0):
            optimizer.zero_grad()
            img1=sample['rawimg_0'][:,:,125:175,125:175].contiguous()
            img2=sample['rawimg_1'][:,:,125:175,125:175].contiguous()
            lab=sample['lab_0'][:,125:175,125:175].contiguous()
            rawimg = torch.cat([img1, img2], 1)

            # B, C, H, W -> B*H*W, C, 1
            rawimg = rawimg.view(rawimg.shape[0], rawimg.shape[1], -1).long()      # B, C, H*W
            rawimg = rawimg.transpose(1, 2).contiguous().view(-1, seq_len*2)  # B*H*W, C, 1
...
if __name__ == '__main__':
    for i in range(10):
        netmodel = 'Transformer_decompms_(1_2_3)' + str(i)
        main(netmodel)
