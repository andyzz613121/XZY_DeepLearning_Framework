...
from model.SegNet import SegNet_AW
...
def main():
    ...
    model_img_pre = torch.load('D:\\Code\\LULC\\Laplace\\result\\Auto_Weight\\Vai_base\\image_model50.pkl').cuda()
    model_dis = torch.load('D:\\Code\\LULC\\Laplace\\result\\Dis_sigmoid\\image_model20.pkl').cuda()
    model_img = SegNet_AW.SegNet_AW(6, 6).cuda()
    model_img = SegNet_AW.add_pre_model(model_img, model_img_pre)
    # model_img = SegNet_AW.freeze(model_img)
    # optimizer = torch.optim.SGD(params=filter(lambda p: p.requires_grad, model_img.parameters()), 
