...
def main():
    ...
    # model_img = SegNetCB(22, 10).cuda()
    # model_img_pre = SegNetCB(3, 10).cuda()
    # model_img = add_conv_channels(model_img, model_img_pre, [19])

    model_img = deeplabv3plus(input_channels=10, out_channels=6).cuda()
    model_hed = torch.load('result\\Edge\\10.17\\CB\\Hed_image_model200.pkl').cuda()  
    ...
    while cur_epochs <= total_epoch: #cur_itrs < opts.total_itrs:
        ...
        for i, sample in enumerate(train_loader, 0):
            ...
            loss = Classification_Loss.loss_with_CB(output, edge, label.long())
            loss.backward()
            optimizer.step()
