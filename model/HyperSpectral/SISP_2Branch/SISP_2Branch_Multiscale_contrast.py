...
class SP_2DDilaConv_MultiScale(nn.Module):
    def __init__(self, input_channels, out_channels):
        ...
        self.SP_conv_3 = DilaConv(16, 16, 2)
        self.SP_conv_4 = DilaConv(16, 16, 4)
        self.SP_conv_5 = DilaConv(16, 16, 8)
        #///////////////////////////////////////////////////////////////////////
        # SP sideout mlps 光谱
        self.SP_Sideout1 = get_mlp(2, [16, out_channels])
        self.SP_Sideout2 = get_mlp(2, [16, out_channels])
...
class SISP_2Branch_MultiScale_contrast(SISP_2Branch):
    def __init__(self, in_channel, out_channel):
        super(SISP_2Branch_MultiScale_contrast, self).__init__(in_channel, out_channel)
        print('Using SISP_2Branch_MultiScale_contrast')
        #///////////////////////////////////////////////////////////////////////
        # self.Net2D = SP_2DDilaConv_MultiScale(1, out_channel)
        self.Net2D = SP_2D_MultiScale(3, out_channel)
        #///////////////////////////////////////////////////////////////////////
        ipt_c_re = in_channel
        ratio = 1
        self.scale = 2
        ...
        self.mlp2_1 = get_mlp(2, [int(ipt_c_re/s), int(ipt_c_re/s)*ratio])
        self.mlp2_2 = get_mlp(2, [int(ipt_c_re/s), int(ipt_c_re/s)*ratio])
        self.mlp3_1 = get_mlp(2, [int(ipt_c_re/(s*s)), int(ipt_c_re/(s*s))*ratio])
        self.mlp3_2 = get_mlp(2, [int(ipt_c_re/(s*s)), int(ipt_c_re/(s*s))*ratio])
        self.mlp4_1 = get_mlp(2, [int(ipt_c_re/(s*s*s)), int(ipt_c_re/(s*s*s))*ratio])
        self.mlp4_2 = get_mlp(2, [int(ipt_c_re/(s*s*s)), int(ipt_c_re/(s*s*s))*ratio])
        self.mlp5_1 = get_mlp(2, [int(ipt_c_re/(s*s*s*s)), int(ipt_c_re/(s*s*s*s))*ratio])
        self.mlp5_2 = get_mlp(2, [int(ipt_c_re/(s*s*s*s)), int(ipt_c_re/(s*s*s*s))*ratio])
    ...
    def MultiScale_SP(self, SP_img, scale_list=[1, 2, 4, 8, 16], samesize=False):
        '''
        Usage:
        ...
        for scale in scale_list:
            ...
            if samesize==True:
                SP_scale = F.interpolate(SP_scale, size=(h, w), mode='bilinear')
            SP_MultiScale.append(SP_scale)
        
        return SP_MultiScale

    def MultiScale_CT(self, vector_ct, act_list1, act_list2, scale_list=[1, 2, 4, 8, 16], samesize=False):
        '''
        Usage:
...
#///////////////////////////////////////////////////////////////////////
    ...
    # MultiScale一起输入_together
    def forward(self, x):
        b, c, h, w = x.size()
        
        x_3d = torch.unsqueeze(x, 1)
        SP_MultiScale = self.MultiScale_CT(center_pixel(x), self.mlp_list1, self.mlp_list2, scale_list=[1,self.scale,self.scale*self.scale], samesize=True)
        SP_MultiScale = torch.cat([x for x in SP_MultiScale], 1)

        # img_ct = center_pixel(x).unsqueeze(1)
        # img_sp = compute_auto_mulply(img_ct, self.mlp1_1, self.mlp1_2, 0)
        # SP_MultiScale = MultiScale_SP(img_sp)
        #///////////////////////////////////////////////////////////////////////
        # Layer 1
        ...
        SP2_GAP = self.GAP2D(SP2).view(b, -1)
        #///////////////////////////////////////////////////////////////////////
        # Layer 3       
        ...
        SP3_GAP = self.GAP2D(SP3).view(b, -1)
        #///////////////////////////////////////////////////////////////////////
        # Layer 4
        ...
        SP4_GAP = self.GAP2D(SP4).view(b, -1)
        #///////////////////////////////////////////////////////////////////////
        # Layer 5
        ...
        SP5_GAP = self.GAP2D(SP5).view(b, -1)
        ...
        return [Total_fuse, SA1_sideout, SA2_sideout, SA3_sideout, SA4_sideout, SA5_sideout, 
                            SP1_sideout, SP2_sideout, SP3_sideout, SP4_sideout, SP5_sideout], [SP1_GAP, SP2_GAP, SP3_GAP, SP4_GAP, SP5_GAP]
