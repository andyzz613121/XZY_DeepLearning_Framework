...
class SegNetCB(SegNet):
    def __init__(self, input_channels, output_channels, pre_train=True):
        super(SegNetCB, self).__init__(input_channels, output_channels, pre_train)
        self.CBfuse_conv = nn.Sequential(
            nn.Conv2d(output_channels*2, output_channels, kernel_size=1), 
            nn.BatchNorm2d(output_channels)           
        )
        self.edgefuse_resblock = BasicBlock(output_channels, output_channels)
    ...
    def forward(self, x, edge):
        ...
        out_convfuse = self.CBfuse_conv(torch.cat([conv_1, edge], 1))
        out_resfuse = out_convfuse + self.edgefuse_resblock(out_convfuse)
        return out_resfuse
        
class SegNetGB(SegNetCB):
    def __init__(self, input_channels, output_channels, pre_train=True):
        super(SegNetGB, self).__init__(input_channels, output_channels, pre_train)
        self.CBfuse_conv = nn.Sequential(
            nn.Conv2d(output_channels+1, output_channels, kernel_size=1), 
            nn.BatchNorm2d(output_channels)           
        )
