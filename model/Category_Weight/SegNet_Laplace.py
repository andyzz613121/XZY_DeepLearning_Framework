...
import numpy as np
class SegNet_Laplace(nn.Module):
    def __init__(self, input_channels, output_channels, zip_channels=0, SE_flag=False):
        super(SegNet_Laplace,self).__init__()
        ...
        #Laplace
        if zip_channels==0:
            self.zipconv_for_laplace = nn.Conv2d(64, 64-3*input_channels, kernel_size=1)
        else:
            self.zipconv_for_laplace = nn.Conv2d(64, 64-zip_channels, kernel_size=1)

        #imgdsm_fuse
        self.imgdsm_fuse = nn.Conv2d(2*output_channels, output_channels, 1)
        
        self.SE_flag = SE_flag
        if self.SE_flag==True:
            self.SE = SELayer(64)
        
        print('SegNet: input_channels: %d, output_channels: %d, zip_channels: %d, SE_flag: %d'%(input_channels, output_channels, zip_channels, SE_flag))
        print('SegNet param init: first init weight by xavier')
    ...
    def forward(self, x, Laplace):
        #encoder
        conv_1 = self.conv_1(x)
        conv_1_copy = conv_1
        conv_1_zip = self.zipconv_for_laplace(conv_1)
        conv_1_Laplace = torch.cat([conv_1_zip,Laplace], 1)
        if self.SE_flag == True:
            conv_1_Laplace = self.SE(conv_1_Laplace)
        conv_1, index_1 = self.pool(conv_1_Laplace)
