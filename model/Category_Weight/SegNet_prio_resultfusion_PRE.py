...
class SegNet_img(nn.Module):
    ...
    def forward_first(self, x, label):
        ...
        sideout_5 = self.CARB_block5(conv_5) #第一层图像路的输出结果
        
        sideout_5 = F.interpolate(sideout_5, size=(x.shape[2], x.shape[3]), mode='nearest')
        out_PRE = torch.argmax(sideout_5, 1)
        # label = F.interpolate(label, size=(pred_5.shape[1], pred_5.shape[2]), mode='nearest')
        dem_weight_E, dis_weight_E, ndvi_weight_E = self.prio.cal_pri_knowledge_weight(out_PRE, label)
        
        self.dem_weight_list5.append(dem_weight_E )
        self.dis_weight_list5.append(dis_weight_E )
        self.ndvi_weight_list5.append(ndvi_weight_E )
    ...
    def forward_second(self, x, prio_features_list1, prio_features_list2, prio_features_list3, prio_features_list4, prio_features_list5, label):
        ...
        conv_5 = self.conv_5(conv_4)
        conv_5_copy = conv_5
        out_PRE = self.CARB_block5(conv_5)
...
class SegNet_prio(nn.Module):
    ...
    def forward(self, x):
        ...
        #decoder
        conv_5 = self.unpool(conv_5p,index_5,output_size=conv_5_copy.shape)
        conv_5 = self.deconv_5(conv_5)
