...
class Fuse_Block(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.sideout1 = Sideout_Block(input_channels, output_channels)
        ...
        self.sideout2 = Sideout_Block(input_channels, output_channels)
        ...
        self.sideout3 = Sideout_Block(input_channels, output_channels)
        ...
        self.sideout4 = Sideout_Block(input_channels, output_channels)
        ...
        self.sideout5 = Sideout_Block(input_channels, output_channels)
...
class SegNet_img(nn.Module):
    ...
    def forward_first(self, x, label):
        ...
        sideout_5 = F.interpolate(sideout_5, size=(x.shape[2], x.shape[3]), mode='nearest')
        pred_5 = torch.argmax(sideout_5, 1)
        # label = F.interpolate(label, size=(pred_5.shape[1], pred_5.shape[2]), mode='nearest')
        dem_weight_5, dis_weight_5, ndvi_weight_5 = self.prio.cal_pri_knowledge_weight(pred_5, label)
        self.dem_weight_list5.append(dem_weight_5)
        self.dis_weight_list5.append(dis_weight_5)
        self.ndvi_weight_list5.append(ndvi_weight_5)

        return sideout_5

    def forward_second(self, x, prio_features_list1, prio_features_list2, prio_features_list3, prio_features_list4, prio_features_list5, label):
        #encoder
        # label = label.unsqueeze(1).float()
        ...
        conv_5 = self.conv_5(conv_4)
        conv_5_copy = conv_5
        out_PRE = self.side_block5(conv_5)
        fuse_5 = self.fuse_block5(conv_5, prio_features_list5)
        # # class_prob = torch.cat([fuse_5[0], fuse_5[1], fuse_5[2], fuse_5[3], fuse_5[4]], 1)
        # # out_5 = (sideout_5[:,0:5,:,:] + class_prob)/2
        # # out_5 = torch.cat([out_5, sideout_5[:,5,:,:].unsqueeze(1)], 1)
        # class_prob = torch.cat([fuse_5[0], fuse_5[1], fuse_5[2], fuse_5[3], fuse_5[4]], 1)
        # out_5 = (sideout_5[:,0:5,:,:] + class_prob)/2
        # out_5 = torch.cat([out_5, sideout_5[:,5,:,:].unsqueeze(1)], 1)
        # # #background保持原先的预测
        # pre_sideout_5 = torch.argmax(sideout_5, 1).unsqueeze(1).repeat(1, 6, 1, 1)
        # background_index = ( pre_sideout_5== 5)
        # out_5[background_index] = sideout_5[background_index]
        out_5_1 = self.fuse_img_prio_conv5_c1(torch.cat([out_PRE[:,0,:,:].unsqueeze(1), fuse_5[0]], 1))
        out_5_2 = self.fuse_img_prio_conv5_c2(torch.cat([out_PRE[:,1,:,:].unsqueeze(1), fuse_5[1]], 1))
        out_5_3 = self.fuse_img_prio_conv5_c3(torch.cat([out_PRE[:,2,:,:].unsqueeze(1), fuse_5[2]], 1))
        out_5_4 = self.fuse_img_prio_conv5_c4(torch.cat([out_PRE[:,3,:,:].unsqueeze(1), fuse_5[3]], 1))
        out_5_5 = self.fuse_img_prio_conv5_c5(torch.cat([out_PRE[:,4,:,:].unsqueeze(1), fuse_5[4]], 1))
        out_5_6 = out_PRE[:,5,:,:].unsqueeze(1)
        ...
        out_CGR = torch.cat([fuse_5[0],fuse_5[1],fuse_5[2],fuse_5[3],fuse_5[4],out_PRE[:,5,:,:].unsqueeze(1)],1)
    ...
    def forward(self, x, label, prio_features_list1, prio_features_list2, prio_features_list3, prio_features_list4, prio_features_list5, forward_flag):
        ...
        if forward_flag == 2:
            return self.forward_second(x, prio_features_list1, prio_features_list2, prio_features_list3, prio_features_list4, prio_features_list5, label)
        if forward_flag == 3:
            return self.forward_third(x)
