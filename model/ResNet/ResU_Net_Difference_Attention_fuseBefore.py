...
class ResU_Net_Difference_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # d3 = self.PAM_Map3(d3_map, d4_map, d3_out) + e2
        d3 = d3_out + e2
        ...
        # d2 = self.PAM_Map2(d2_map, d3_map, d2_out) + e1
        d2 = d2_out + e1
        ...
        # d1 = self.PAM_Map1(d1_map, d2_map, d1_out) + x_unpool
        d1 = d1_out + x_unpool
        ...
        # out4 = self.Pre_4(out)  
        # out5 = self.Pre_5(out)  
        # out = torch.cat([out0, out1, out2, out3, out4, out5], 1)
        d1_map = F.interpolate(d1_map, size=(d1.shape[2]*2, d1.shape[3]*2), mode='bilinear')
        d2_map = F.interpolate(d2_map, size=(d1.shape[2]*2, d1.shape[3]*2), mode='bilinear')
        d3_map = F.interpolate(d3_map, size=(d1.shape[2]*2, d1.shape[3]*2), mode='bilinear')
        d4_map = F.interpolate(d4_map, size=(d1.shape[2]*2, d1.shape[3]*2), mode='bilinear')
