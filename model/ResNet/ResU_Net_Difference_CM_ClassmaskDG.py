...
class Global_Error_Attention(nn.Module):
    def __init__(self, class_num, out_channels):
        ...
        self.proj = nn.Conv2d(class_num, out_channels, 1)
    ...
    def forward(self, map, cm, feature_map):
        ...
        cm = cm.view(1, self.class_num, self.class_num).permute(0, 2, 1)
        cm = cm.repeat(batch_size, 1, 1)
        
        print(map.shape, cm.shape)
        category_weight = torch.bmm(map, cm)
        category_weight = category_weight.permute(0, 2, 1).view(batch_size, -1, height, width)
        out = self.alpha * feature_map * self.proj(category_weight) + feature_map
...
class ResU_Net_Diff_Global_Attention(nn.Module):
    def __init__(self, input_channels, num_classes, pre_train=True):
        ...
        self.Sideout4 = CARB_Block(256, num_classes)
        self.Sideout3 = CARB_Block(128, num_classes)
        self.Sideout2 = CARB_Block(64, num_classes)
        self.Sideout1 = CARB_Block(64, num_classes)
        ...
        self.GEA4 = Global_Error_Attention(num_classes, 256)
        self.GEA3 = Global_Error_Attention(num_classes, 128)
        self.GEA2 = Global_Error_Attention(num_classes, 64)
        self.GEA1 = Global_Error_Attention(num_classes, 64)
